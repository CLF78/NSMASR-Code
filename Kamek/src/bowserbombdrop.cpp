...
int dDroppedBomb::onCreate() {
	...
	KoopaJunk.category1 = 3;
	KoopaJunk.category2 = 0;
	...
	// These structs tell the collider what to collide with - these are from Koopa Troopa
...
int dDroppedBomb::onExecute() {
	...
	bool ret = this->outOfZone(this->pos, (float*)&rect, this->currentZoneID);
	if (ret) {
		this->Delete(1);
...
int dBombDrop::onDelete() {
	return true;
}

int dBombDrop::onDraw() {
	return true;
}
...
int dBombDrop::onExecute() {
	...
	if (dFlagMgr_c::instance->active(eventA)) {
		create(WM_SMALLCLOUD, eventA+1, &pos , &rot, 0);
		HackyBombDropVariable = true;
		dFlagMgr_c::instance->set(eventA, 0, false, false, false);
	}

	if (dFlagMgr_c::instance->active(eventB)) {
